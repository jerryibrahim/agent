...
**Do not change** the CSS, class names, DOM structure, toolbar layout, sidebar styling, or
the `<!-- nav:inject -->` comment after `<body>`. The template enforces these rules:
...
control placement, the `<!-- nav:inject -->` comment after `<body>`, or any other structural
element. The template is the single source of truth for the interactive page layout.
...
**Do not change** the CSS, class names, or content container styling. The template enforces
the dark theme (`#0f172a` background, `#e2e8f0` text) and consistent typography.

**Navigation**: The template contains no built-in navigation bar. Instead, it includes a
`<!-- nav:inject -->` comment placeholder right after `<body>`. This placeholder is used
by the `visualize-index` skill to inject shared navigation when the page is collected into
a multi-project gallery. Do not remove or modify this comment. Do not add any topbar,
breadcrumb, or navigation elements — the page should work standalone without navigation.
...
### 4a. Structural validation — template conformance

For each generated HTML file, read **both** the generated file and its canonical
template (from `templates/` relative to this skill file). Compare the structural
skeleton — everything **outside** the project-specific content — and verify they
match exactly.

**Validation procedure for each file:**

1. Read the canonical template (`templates/index.html`, `templates/interactive.html`,
   or `templates/deep-dive.html`).
2. Read the generated file.
3. Extract the **structural skeleton** from both: the `<!DOCTYPE>` declaration,
   `<html>` and `<head>` tags, the full `<style>` block, the `<body>` tag and
   `<!-- nav:inject -->` comment, and the outer `<div>` containers (class names,
   nesting, order). Ignore the project-specific content inside these containers
   (slide data, scenario data, analysis text).
4. Diff the two skeletons. Any differences are violations that must be fixed:
   - **Extra elements** not in the template (e.g., topbar divs, breadcrumbs,
     back-links) → remove them.
   - **Missing elements** from the template (e.g., `<!-- nav:inject -->`,
     CSS rules, container divs) → add them.
   - **Modified CSS** (e.g., multi-line/tabbed format instead of compact
     single-line, added or changed rules) → replace with the template's CSS.
   - **Changed class names, DOM structure, or nesting** → rewrite to match
     the template while preserving project-specific content.
5. If any fixes are needed, rewrite the file and re-validate.

**The generated file must be identical to the template except for:**
- `{{PROJECT_NAME}}` replacements
- Content inside designated data sections (`SLIDES` array, `SCENARIOS` array,
  `<div class="content">` body)
- Slide entries added or removed to match generated SVGs
...
3. Report to the user what was generated and where, including validation results.
